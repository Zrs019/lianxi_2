...
# 使用函数处理数据
result_df = process_cooling_data_new_format("测试.xls")  # 修改为你的测试文件
...
if not result_df.empty:
    ...
    # 保存结果
    result_df.to_csv('中心站A1历史记录报表时间_总冷量_新格式.csv', index=False, encoding='utf-8-sig')
    print(f"\n结果已保存到 '中心站A1历史记录报表时间_总冷量_新格式.csv'")
    ...
    print(f"📂 文件保存在：{current_dir}")
    
    # 列出生成的文件
    output_files = ['中心站A1历史记录报表时间_总冷量_新格式.csv']
    for filename in output_files:
        if os.path.exists(filename):
            full_path = os.path.join(current_dir, filename)
            print(f"\n📄 {filename}:")
            print(f"  完整路径：{full_path}")
            print(f"  快速访问：file://{full_path}")
        else:
            print(f"\n⚠️ {filename}：文件未生成")
else:
    print("处理失败：未提取到有效数据")