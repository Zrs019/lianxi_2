...
# 3) 优化核心
# =========================
def optimize_dispatch_fast(q_load, price, chillers_all, ice_unit, params, p_cap=None):
    import pulp
    import pandas as pd
    ...
    # ===== 1) 冷机等效聚合（提速关键）=====
    Qmax_total = sum(v["Qmax"] for v in chillers_all.values())

    # 取75%点做等效COP（工程上常用，精度/速度折中）
    # 也可改成加权平均: sum(Qmax_i*COP75_i)/sum(Qmax_i)
    COP_eq = sum(v["Qmax"] * v["cop_75"] for v in chillers_all.values()) / Qmax_total

    m = pulp.LpProblem("Ice_Storage_Fast", pulp.LpMinimize)

    # 变量
    q_ch = {h: pulp.LpVariable(f"q_ch_{h}", lowBound=0, upBound=Qmax_total) for h in hours}
    p_ch = {h: pulp.LpVariable(f"p_ch_{h}", lowBound=0) for h in hours}
    ...
    for h in hours:
        # 电制冷功率线性化
        m += p_ch[h] == q_ch[h] / COP_eq

        # 仅谷段允许蓄冰（大提速）
        if h not in [0,1,2,3,4,5,6,7]:
            m += q_ice_ch[h] == 0
            m += p_ice_ch[h] == 0
        else:
        ...
        # 冷量平衡
        m += q_ch[h] + q_dis[h] == q_load[h] + q_ice_ch[h]

    # SOC周期
    # 先固定初值（比如50%）
    m += soc[0] == 0.5 * ice_unit["E_ice_max"]
    ...
    # 功率与峰值
    for h in hours:
        m += p_total[h] == P_base[h] + p_ch[h] + p_ice_ch[h]
        m += p_peak >= p_total[h]
    ...
    m += pulp.lpSum(price[h]*p_total[h]*dt for h in hours) + daily_demand_factor * p_peak

    # 求解
    # solver = pulp.PULP_CBC_CMD(msg=False, threads=0)  # threads=0自动多线程
    # m.solve(solver)
# 关键：用 fast 模型函数 + 时间限制
    solver = pulp.PULP_CBC_CMD(
        msg=True,          # 打开日志看进度
        timeLimit=60,      # 最多60秒
        gapRel=0.01,       # 1%相对gap就停
        threads=0
    )
    m.solve(solver)
    print("Solver status:", pulp.LpStatus[m.status])
